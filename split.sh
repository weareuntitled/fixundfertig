...
# Usage:
#   ./split_pages.sh
#   ./split_pages.sh path/to/pages.py

echo "Split Pages: Start"

# Optional: make sure we are in a project root
if [[ ! -f "pyproject.toml" && ! -f "requirements.txt" && ! -d ".git" ]]; then
  echo "Hinweis: Kein klares Projektroot erkannt. Bitte im Projektroot ausführen."
fi

# Create scripts dir
mkdir -p scripts

# Write the Python splitter
cat > scripts/split_pages.py <<'PY'
...
# If pages/ already exists, abort unless user sets FORCE_SPLIT_PAGES=1
# This avoids overwriting existing work.
if [[ -d "pages" ]]; then
  if [[ "${FORCE_SPLIT_PAGES:-}" != "1" ]]; then
    echo "Fehler: Ordner ./pages existiert bereits."
    echo "Abbruch, um nichts zu überschreiben."
    echo "Wenn du überschreiben willst: FORCE_SPLIT_PAGES=1 ./split_pages.sh"
    exit 1
  fi
fi

echo "Split Pages: Running Python splitter"
if [[ $# -ge 1 ]]; then
  python3 scripts/split_pages.py "$1"
else
  python3 scripts/split_pages.py
fi

echo "Split Pages: Compile check"
python3 -m compileall . >/dev/null

echo "Split Pages: Done"
echo "Nächster Schritt: In deinem Startscript (wo ui.run() ist) sicherstellen, dass 'import pages' ausgeführt wird."
PYTHON_SPLIT_END
